...
suspend fun updateLocalFeed(feed: Feed, progressCB: ((Int, Int)->Unit)? = null) {
    ...
    fun createEpisode(feed: Feed, file: DocFile): Episode {
        ...
        try {
            ...
            MediaMetadataRetrieverCompat().use { mmr ->
                ...
                try {
                    PodciniApp.getAppContext().contentResolver.openFileDescriptor(file.uri, "r")?.use { pfd ->
                    ...
                    LogsFor(TAG, item.id, "MediaMetadataRetrieverCompat failure: ${e.message}")
                }
            }
        } catch (e: Exception) {
    ...
    fun traverseAll(uri: Uri, docId: String? = null) {
        ...
        val startDocId = when {
            docId != null -> docId
            DocumentsContract.isDocumentUri(PodciniApp.getAppContext(), uri) -> DocumentsContract.getDocumentId(uri)
            else -> treeId
        }
        ...
        PodciniApp.getAppContext().contentResolver.query(childUri, arrayOf(
    ...
    if (!documentFolder.exists()) throw kotlinx.io.IOException("Cannot read local directory $uriString. \nTry re-connecting the folder on device")
    ...
    getFeed(feed.id)?.let { f->
        if (mustReportDownloadSuccessful(f)) {
            DownloadResult.logDownloadResult(DownloadResult(f, DownloadError.SUCCESS, true, ""))
